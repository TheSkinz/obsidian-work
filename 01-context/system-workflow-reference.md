...
| **Agent / Review Loop** | Scheduled, 8th of month ~4am (`vault-review-loop`), since 2026-08-21; also on demand — you say "run the Vault Review Loop" | Operational core: `02-facilities/`, `04-knowledge/`, pricing/SOP/safety/field-execution content | Picks one item, writes a review/contradiction/question note with a Decision checklist. Never edits canonical content without your approval. | High — propose-only, and that is what makes an unattended run safe: it writes a review note and a queue row, never canonical content. It is explicitly allowed to write nothing in a month with nothing worth deciding. |
...
| **Skill-Drift Loop** | Scheduled, 1st of month ~3am (`vault-skill-drift-loop`), since 2026-07-25; also on demand — you say "run the Skill-Drift Loop" | Reads `~/.claude/skills/` + vault; writes one review note + a `drift/YYYY-MM` config-repo branch | Detects skills contradicting vault truth, each other, or reality; packages fixes as an unmerged proposal branch. You merge or discard. | Medium — unattended, but propose-only: it writes to a branch and never merges, so you are still the gate. The old "must stay manual, config-repo git authority is not pre-granted" reasoning was retired 2026-07-25 as a false premise — `auto` mode permits those verbs, which is why the run fired unattended and pushed `drift/2026-07b`. See [[vault-skill-drift-loop-spec]]. |
...
Full specs for what still runs: [[vault-agent-loop-spec]], [[vault-skill-drift-loop-spec]], [[vault-consolidation-loop-spec]], plus [[vault-idea-loop-spec]] — which is now an **on-demand procedure, not a loop**, despite its filename.

**Retired 2026-09-08:** the capture and pre-staging specs are in `archive/`. Both described work that stopped on 2026-08-21 and neither held any live procedure — their two load-bearing pieces, the Terminal-Note Sweep and the Three-Outcome Routing Model, moved to [[knowledge-system-governance]] first. The idea spec was kept rather than archived with them because its research procedure and seed lifecycle are still used.

**Not a loop, but scheduled:** `vault-audit-remeasure` is a one-time task set to fire **2026-09-22**. It re-measures the 61% system-maintenance figure a month after the 2026-08-21 loop retirement, appends its answer to the audit note's Execution log, and is the only thing carrying that prediction — the dormant-trigger registry that would have held it was retired the same day. Read-only apart from that append.
...
**A fourth trigger now runs outside the loops, and it is defect-triggered by construction (2026-09-06).** `tools/notify_grok_bot.py` fires from a `pre-push` hook and notifies a Grok Bot routine, which pulls the read-only vault clone its Bots cite domain truth from. It exists because that clone otherwise goes stale between sessions, and a Bot citing a stale clone is the same defect as a session reading a stale `01-context/` — the failure the rig-in drift produced on 2026-09-05, where a citation audit quoted the superseded line **verbatim and correctly**. It never blocks a push: every failure path exits 0, the request itself runs in a **detached child** so the push does not wait for it at all (fixed 2026-09-07 — the endpoint blocks until the run is queued and slows as runs pile up, measured 1.0s idle → 21.7s → 53.3s, so the original 5-second timeout failed on every push after the first in a session and printed a misleading `webhook unreachable`), and unsetting `GROK_BOT_WEBHOOK_URL` or `GROK_BOT_WEBHOOK_KEY` is the off switch. The outcome now lands in `.grok-bot-last` (gitignored, one line, overwritten) rather than in the push output, because by the time it is known the push has finished — **that file is the only place a failure will show, so check it if the clone looks stale.** It also pulls always but speaks only when something under `01-context/` moved, which is the same quiet-unless-defective posture the surviving loops were narrowed to. Full setup in `07-llms/grok/bot-setup/SETUP.md`.
...
| **usadebusk-fieldpm** | Field project management for a job that is running or being reported. Whether one is live is answered by `active-jobs.md`, not by the skill — its `status: dormant` field and mobilization flag were removed 2026-09-04 | `/setup`, `/extract`, `/log`, `/email`, `/status`, `/report`; service receipts, shift notes, payroll, job progress | usadebusk-core; usadebusk-equipment mid-job if pig sizing comes up |
...
| **adhd** | Parallel divergent ideation — spawns isolated branches under different cognitive frames, scores and clusters, flags traps, deepens survivors. Generates options, never decides them | `/adhd` only (`disable-model-invocation: true`, so it is not model-loadable); an open-ended design, architecture, naming, approach or fuzzy-diagnosis problem | Standalone; survivors hand off to **idea-triage** |
...
2. **Nothing picks it up on a schedule any more** (idea research stopped 2026-08-21, same reasoning as capture). Seeds accrue; say "research this seed" in a session and the same bounded pass runs on demand — prior art, existing tools, what this vault already has — ending in a review note in `06-reviews/` with an evidence-backed recommendation (sound / trap / premature / already covered). The research logic lives in [[vault-idea-loop-spec]] and is unchanged; only its trigger is gone.
3. The seed's own `status` flips to `researched` with a link to the review note — and **must not stop there.** Disposition happens in the same pass: the seed leaves at a terminal status, at `gated` with a `revisit-trigger:`, or at `decided-blocked`. On 2026-09-08 all ten seeds resting at `researched` turned out to be already ruled, eight of them already applied.

...
- `archive/` — **listed in `.gitignore`, but partly tracked anyway.** Git keeps tracking what it already tracks, so the ignore rule only ever applied to files added after it. Checked 2026-09-03: 47 files tracked, 17 untracked. A file here may therefore exist only on disk *or* be fully recoverable, and nothing about the path tells you which. Never treat deletion from `archive/` as recoverable without running `git ls-files` on the specific file first.

...
**[2026-07-01] | System — Vault↔Skill knowledge reconciliation pass:** Reconciled migrated Master-Reference-Module knowledge against the current `usadebusk-*` skills (skills = source of truth; skills carry corrections made after the 2026-04/06 migration). Corrected: **TriMax architecture** — vault had it as a single-pump unit; it is a **Triple** (3 independent pump/engine assemblies, L/C/R = 1/2/3, sharing one 3,000 gal clean + one 2,000 gal dirty tank, 3 operator stations). Fixed **Second TriMax** — filtration scaling is conditional (not always 1 shared press) and each assembly runs its own direction (no cross-assembly direction constraint; the prior "both units pig the same direction" [2026-03] is superseded). Added **Double Pumper** unit and the **dual-pumper terminology ban** / Triple-vs-Double-vs-Second naming discipline. Reconciled TriMax operating pressure to 100–400 PSI typical / 600 absolute / >500 over-pressure checklist. Added: L/C/R shift-summary convention (field-ops), the 4-part Mob/Demob costing formula + stand-by-two-lines rule (estimating), looped-circuit transit ~6–30 min. **Preserved untouched:** chloride ppm limits (still DRAFTED-NOT-CANONICAL), smart-pig vendor list, extra tube dimensions, rate tables (already matched). Files: equipment-library, field-operations, estimating-pricing, process-flow, 01-context/estimating-approach.

**[2026-07-01] | Equipment — Double Pumper build resolved:** The "Sea-Can Double Pumper" flag from the reconciliation pass above is resolved, per Jesse — not a discrepancy. It's a single 48' trailer with a sea-can container retrofitted onto the bed, housing the two engine/pump/valve assemblies. Both descriptions (fleet name and skill's "single 48' trailer") are correct; `equipment-library.md` updated to state the sea-can build explicitly.

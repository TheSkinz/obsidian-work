...
| A flow test whose numbers don't behave — a weak Δ, a reading out of family with the other circuits, a ladder step that doesn't track | Looks like a coil that cleaned poorly, a transcription error worth reconstructing, or a data-integrity finding | **About 60% of projects carry a flow-test error, usually one aspect of one pass, usually minor** (Jesse, 2026-09-02). The causes are physical and mostly invisible to the tech: a coil not fully packed when the test ran, a throttled effluent route, a blocker valve not reopened after a smart pig run, or a valve whose ball is not travelling fully open while indicating open. **The tech often cannot explain it, and that is expected.** So a bad reading is a failed measurement, not a finding about the coil, and not a puzzle to solve with arithmetic — do not build forensics on it, do not reconstruct what it "should" have read, and never read a weak Δ as a circuit that cleaned less well. Completion rests on effluent duration and sustained clarity; the flow test corroborates (`manual/10-verification-and-completion.md` §10.2). Escalate the instance to Jesse and decide case by case. **Internal only — none of this reasoning ever appears in a customer document; the report omits a suspect pass rather than explaining it** |

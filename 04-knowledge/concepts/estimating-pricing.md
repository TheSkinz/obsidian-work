...
Rig-In / Rig-Out: Fixed events; **rig-out matches rig-in** at the whole figure including adders. **Rig-in is 6 hrs — use the default, do not build a method for this line** (Jesse, 2026-09-05). Two departures only: **8 hrs** when connections are elevated above 40 ft *and* the pumper runs double or triple mode (both, not either), and **12 hrs** only at the extreme — pumper roughly 80 ft from the heater base *and* connections 40+ ft up. Elevation and mode are background, not a calculation; do not turn them into a hose count. **This replaces the four-tier model** (Small 4 / Moderate 6 / Large 8 / Very large 12, tiered from three multiplying drivers) retired 2026-09-05; run distance is no longer an independent driver. Canonical: `01-context/estimating-approach.md`. Rig-Over between pass sets = `ceil(passes ÷ mode) − 1` (mode = passes cleaned per set: double 2, triple 3), ~1 hr with launchers/receivers pre-installed on the added passes else ~2 hr; on silence take the 1-hr figure. **Smart Pig: 2 hrs per pass when elected** (Jesse, 2026-09-06, replacing a flat 4) — an estimate only, and quoted jobs will disagree with it without that being a defect to reconcile. **On a multi-heater mobilization the FIRST heater takes the full tier and subsequent heaters in close proximity take a reduced figure** (Jesse, 2026-08-23 — 6 vs 4 on DSP#26100), because the pumper stays partially rigged; that is why `Rig-Over` reads 0 on a job where the equipment moves between heaters. **Pre-inspection pigging on an already-decoked coil is a task allowance, not footage ÷ rate** — see `usadebusk-estimating` and [[DSP26100]].

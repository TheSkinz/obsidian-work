...
- **No cross-quote view of what has actually been charged.** Bill rates are scattered one-per-quote-note, so "what did we charge at Baytown last time" means opening files individually. The eventual fix is a generated rate-history rollup on the pattern of `tools/estimating_rollup.py`. **Not built** — with four quote notes it isn't worth a tool, and the quote notes don't yet share a consistent rate-table heading. **Trigger:** roughly a dozen quote notes under a settled heading convention.
  ...
  **Design constraint added 2026-07-26 — the rollup must not reduce a facility to one rate.** Section 3 above already establishes that rates are a contract property and that one site can carry several concurrent contracts; this adds the *reason the spread persists* and what it means for the tool. Jesse, 2026-07-26: a site like ExxonMobil Baytown is not one buyer. Bids go through different groups and divisions, and where USADeBusk has to be more competitive the rate moves. $30/hr support unit will be right for the majority of bids, but "the majority" is not "the rate." So the spread is not drift to be reconciled away — it is a live commercial variable, and a rollup that averaged or last-wrote across a facility would manufacture a number that was never quoted to anyone.

  Two implications for whenever it gets built. **(1) Show the spread, don't collapse it.** The unit of truth is the quote note; the rollup date-orders and compares, it does not resolve. A `House standard` column alongside the per-quote columns is the honest shape — it states the default without claiming any customer agreed to it. Worked example now live at `02-facilities/ExxonMobil/Baytown-TX/_facility.md` under `## Rate History — Baytown`. **(2) The grouping dimension does not exist yet as data.** Nothing in quote frontmatter records which group or division a bid went through, so the rollup could not segment by the variable that actually explains the spread. If that segmentation is wanted, the field has to be captured at intake — reconstructing it from old quotes later is guesswork.

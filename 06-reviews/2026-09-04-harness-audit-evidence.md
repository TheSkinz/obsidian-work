...
| `usadebusk-fieldpm` is `status: dormant`; "measure whether it wakes" | ~~**Never slept.** The frontmatter is stale.~~ **This audit's answer was itself wrong — corrected 2026-09-04.** The execution numbers hold (`render_job_report.py` 123 runs, SKILL.md read in 17 sessions), but `status: dormant` was **accurate**: `01-context/active-jobs.md` reads "Active: *None currently*" | The audit conflated *the skill is used* with *a job is active*. The frontmatter only ever claimed the latter, and the 123 runs are post-job report production, which the skill's own description covers. The real defect was elsewhere — see §A |
...
| `usadebusk-fieldpm` | 19,743 B SKILL.md, ~330 KB directory. `render_job_report.py` **123 runs**, `extract_ticket_breakdown.py` 27, both last today. **8 sessions loaded the skill *and* ran its scripts**, 8 ran scripts without it (mostly back-test and regression), 5 loaded without running. *measured (tool channel, floor)* | **Unlike `vault-ingest`, this skill does reach its work.** `status: dormant` was accurate — `active-jobs.md` reads "Active: *None currently*". The defect was a hand-maintained duplicate: the frontmatter restated job state that `active-jobs.md` already owns, and it sat **a month stale**, naming USA26038 as last-active through both the USA26041 and CAD26001 jobs | **BOTH FIELDS REMOVED 2026-09-04 (Jesse).** `status:` deleted — nothing reads it; fieldpm was the only skill carrying one. Description now points at `active-jobs.md` and states no job state of its own. Propagated to `01-context/system-workflow-reference.md` and `workflow-map.md`, which both still asserted dormancy. `30f829f` |
| — its `disable-model-invocation` lifecycle | The SKILL.md instructed flipping the flag off at mobilization and back on at demob. Practised **once** (USA26038 — off `12ea767`, back on `0ecf823`), and for **neither** August job. *measured (git)* | Through both un-flipped mobilizations the work still landed — sessions read SKILL.md from disk and ran the scripts (24 runs on the F-501 report, 50 on CAD26001's). **A documented procedure with a 1-in-3 record is Headline 2 in miniature, inside a skill rather than a CLAUDE.md** | **FLAG DROPPED 2026-09-04 (Jesse).** Auto-fire exposure measured first: 1.6% of 185 sessions match a trigger phrase in the first human turn, 6.5% name a job number (a floor — skills can fire on any turn). Description given explicit negative scope so a job number, a job sheet or a past job does not pull it in. Reachability confirmed live in-session. `30f829f` |
| `usadebusk-vault-ingest` | 33,706 B / 602 lines — 2nd largest. **38 commits — 2nd most-maintained.** `disable-model-invocation: true`. **Zero ingestions in the corpus.** All 13 sessions that read it were skill maintenance, schema reference, or drift-loop audits. F2's regression fixture. *measured (tool channel + git)* | **The cause is reach, not quality.** The flag (added 2026-07-07) removes a skill from model invocation entirely. On 2026-07-26 Jesse said *"Ingest this project"*; that session wrote **15 vault files including four heater cards** with the governing skill never loaded — the output conformed anyway because the schema authority lives in the vault, where it was reachable | **FLAG DROPPED 2026-09-04 (Jesse).** Description now names the prose trigger alongside the slash commands, with an explicit negative scope so it does not fire on merely reading a document. Frontmatter verified to parse — `usadebusk-fieldpm` once had this same flag silently ignored by an unquoted description. Confirmed live: the skill appeared in this session's available-skills list immediately after. `d1b1dd1` |
| — its 227-line heater-card schema mirror | Declared in-file: `<!-- Derived from 04-knowledge/_canonical-heater-card.md. Keep in sync. -->`. All 9 shared tables diffed: **zero drift**. Only **4 of 38 commits** ever changed a table row, none since 2026-08-21. *measured (git)* | Section bodies are **not** a pure copy — they carry blank template rows, authoring rules, and dated rulings (DQ-017, DQ-026, the `Condition`-column semantics) that the exemplar does not hold | **LEFT IN PLACE 2026-09-04 (Jesse).** A plan to replace it with a pointer was withdrawn — see the correction in the log |
...
**`usadebusk-vault-ingest` is the worked example, and it is the sharpest instance of Headline 1 in the
audit.** On 2026-07-26 Jesse asked, in prose, for exactly the thing this skill exists to do — *"Ingest
this project."* The session converted the documents with MarkItDown and wrote 15 vault files including
four heater cards. The skill was never loaded, because the flag had made it invisible 19 days earlier.
The work came out conforming regardless, because the schema authority it mirrors lives in the vault
where an agent can reach it — which is also why the failure was invisible for six weeks. **Flag dropped
2026-09-04.** The same flag still sits on `adhd`, `idea-triage` and `usadebusk-fieldpm`; `fieldpm` is
the one worth looking at next, since its scripts ran 123 times while the skill itself stayed unreachable.
...
> **Fixed 2026-09-04 on Jesse's verdict.** One shared resolver (`hooks/usadebusk-hook-cwd.mjs`) now serves
...
| 2026-09-04 | **Fourth verdict: `usadebusk-vault-ingest` flag DROPPED, schema mirror KEPT.** The skill had zero measured ingestions not because it is bad but because `disable-model-invocation: true` made it unreachable — proven by the 2026-07-26 session that wrote 15 vault files on a prose "ingest this project" without it. Description rewritten to name the prose trigger and carry a negative scope; frontmatter verified to parse; reachability confirmed live in-session. `d1b1dd1` | Claude |
| 2026-09-04 | **Second self-correction: my case for stripping the schema mirror was built on a miscount.** I reported that "a large share" of the skill's 38 commits were spent syncing the mirror, generalising from three commit subjects I had quoted. Counted properly: **4 of 38 changed a table row, and none since 2026-08-21.** Diffing full section bodies rather than table headers also showed the mirror is not a copy — it carries blank template rows, authoring rules, and the DQ-017 / DQ-026 / `Condition`-column rulings that the exemplar does not. The proposed strip would have deleted domain truth to save a sync burden that is not being paid. Jesse re-ruled on the corrected numbers: leave it. | Claude |
| 2026-09-04 | **Owed to the next regression session.** The replay guard fired on `d1b1dd1` — its first genuine firing ever, naming F2, and the working proof of the cwd fix. The edit was judged non-substantive by the guard's own definition (no number, rule or ambiguity changed inside the skill body; F2 loads the skill from disk either way) and committed with that reasoning recorded rather than waved through with `replay-ok`. **F2's fixture note still states the skill "cannot be loaded via the Skill tool" — false as of `d1b1dd1`.** A replay run under the stale note would set up the wrong loading conditions. | Claude |
| 2026-09-04 | **Fifth verdict: `usadebusk-fieldpm` — `status:` and the lifecycle flag both removed.** Job state now points at `01-context/active-jobs.md` rather than being restated in the frontmatter, where it had sat a month stale. The flag was a lifecycle control, not a blanket gate, and its documented mob/demob flip had a 1-in-3 record. Auto-fire exposure measured before removal (1.6% trigger-phrase, 6.5% job-number, first-turn floor); description given explicit negative scope. Propagated to `01-context/system-workflow-reference.md` and `workflow-map.md`. Reachability confirmed live. `30f829f` | Claude |
| 2026-09-04 | **Third self-correction: the audit's fieldpm answer was wrong.** It reported "Never slept — the frontmatter is stale." `status: dormant` was accurate; `active-jobs.md` reads "Active: *None currently*", CAD26001 demobbed 2026-08-30 and USA26040 is postponed. The audit conflated *the skill is used* with *a job is active* — the frontmatter only claimed the latter, and the 123 script runs are post-job report production. A follow-on claim, that fieldpm was "the same shape as vault-ingest," was also wrong: vault-ingest's flag was a blanket 2026-07-07 sweep, fieldpm's was a deliberate lifecycle control. Both corrected in the premises block and in §A. | Claude |
| 2026-09-04 | **Owed to the next regression session, now two items.** `f3-receipt-input.md` discusses fieldpm's Skill-tool reachability as a live constraint; it is not one as of `30f829f`, the same way `f2-ingest-input.md` went stale under `d1b1dd1`. The replay guard fired on both commits, naming F2 and F3 — its second and third genuine firings, all three today. Both edits were judged non-substantive by the guard's own definition and committed with that reasoning recorded rather than hatched with `replay-ok`. | Claude |
| 2026-09-05 | **Regression session executed — F1, F4, F5 replayed blind, then F6 twice.** Rubrics pre-registered and committed before every dispatch. F5 (8 commits behind, oldest baseline in the battery) passed with all five figures exact and declined to extrapolate an exception it could not justify. F4 passed all eight keys plus the SOP voice split. F1 passed, then was **replayed a second time and passed by deriving rather than reciting** — see the next row. F6 failed key 5, was not re-cut, and was later re-cut under the corrected rig-in rule with that line ruled unkeyed. Commits `ee4c39a`, `f48ae13`, `b9a4cad`. | Claude |
| 2026-09-05 | **The audit's own fixture had leaked into the skill it tests, and the blinded run disclosed it unprompted.** `usadebusk-estimating` carried a worked example that *was* the F1 fixture — 4 passes, 1,120 ft, mode 3, 100 ft/hr — plus F1's intake item quoted verbatim with its $552 swing priced out. Four of F1's nine diff keys were readable out of the skill under test, so F1 passed by reciting and **could not detect drift in them**. Recast onto colliding-with-nothing values (`328189f`); F1 then reproduced $552 from job data and cross-checked it by rebuilding the mob from scratch. **Transferable lesson, now in `regression/README.md`: sweep for shapes and descriptions, not just values** — F6's crib was "an 8 ft walk-up on twelve circuit lines", which no value-collision check catches. | Claude |
| 2026-09-05 | **Rig-in tier machinery struck; two lines ruled unkeyed.** Jesse did not recognise the fixture heater or the worked illustration built from it, and gave his actual method — default 6, 8 when elevated above 40 ft *and* double/triple mode. The skill's ~28 lines of tier machinery **had been producing 12 where his method gives 8, a 50% overstatement standing since 2026-08-15**, and was elaborate enough that nobody could see it. Hard pig quantity and rig-in/rig-out are now explicitly unkeyed and variable — score the reasoning, never the figure. `b449e63`, `f05a346`, `1f8aea9`. | Claude |

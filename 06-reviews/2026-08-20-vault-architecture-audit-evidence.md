...
| `sharepoint_export.py` | 20 | 08-11 | Writes to `_OUTPUTS/sharepoint`, which exists; referenced in commits through 08-20 | **Keep.** Live and customer-adjacent. Note its `--check` was green throughout the 2026-08-11 incident while the library held the wrong file — DQ-016 is the fix and is still open. **CORRECTION 2026-09-07: DQ-016 is closed `closed-unactioned` and there is no fix.** Company policy blocked third-party access to the M365 tenant, and DQ-016's design required a REST read of the live library. The keep-recommendation stands — generation is local and still works — but the gap this row names is now permanent and unguarded, and the script's drift-prevention claim was withdrawn the same day. See [[m365-access-boundary]] |

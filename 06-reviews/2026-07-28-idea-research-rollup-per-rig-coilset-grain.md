...
breakdown for CAD25004: "Trimax 5 (coils 2/3/4, triple mode) = 6 rig-in / 48 pig / 6 smart /
...
| 2026-08-21 | **Unparked and half-ruled — recommendation (a) approved, (b) applied, (c) deferred to Phase 2** | Jesse (ruling) / Claude (Opus 5) | Forcing function arrived: CAD26001 mobs 08-25 on `7-1-F-1`, the one card with per-coilset history, Jesse on it as operator. **(a) approved as an additive optional `## Coilset Durations` section**, not a change to `## Task Durations` — this note's evidence pointed the right way but the placement matters more than it knew: `estimating_rollup.py`'s `table_rows()` never resets `seen_header`, so a second table inside the parsed section would have had its header eaten as a data row while `check_durations_header` stayed green, since it reads only the first row under the heading. A separate `##` section avoids both. Column set is close to this note's proposal (Job # · Coils · Rig · Mode · Circuit ft · Rig-In · Pig · Smart Pig · Rig-Over · Rig-Out · Total · Coil condition · Flag); a dry-run against both Syncrude jobs reconciled exactly and forced three corrections — Stand-By dropped as per-rig, Rig-Over added, and `Circuit ft` pinned to footage **one pig travels** rather than the set total. **(b) applied** — `usadebusk-vault-ingest` gained a capture-at-ingest step, which this note correctly identified as the load-bearing half. **(c) deferred to Phase 2**, unchanged in substance: hand-migrate `7-1-F-1` only; `HF-0011` stays blended; `F-802` and `H-102B` are per-rig totals and narrative, not coilset grain. **Point 5's bundling held up** — the Pig Specifications `Condition` column question dissolved rather than needing its own ruling, because a new section carries its own coil-condition column at no cost to the lint-locked header. Recorded on DQ-017, which stays open for Phase 2/3. |

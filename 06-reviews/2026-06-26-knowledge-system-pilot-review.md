...
| `CAD25004.md` | Job report import with unresolved routing, Canadian/Syncrude facility data, and blank tags. | Treat as source/job-report candidate. Create facility/customer scaffolding only after confirming whether `CAD25004` or in-body `CAD25002` is the governing job number. | Yes. Job number conflict and new client/facility routing affect canonical records. |
...
Start with `CAD25004.md` because it is operationally valuable and currently has explicit unresolved routing. The first action should be a review note, not a canonical edit.

Recommended first follow-up:

1. Create a contradiction/review note for the job-number mismatch: frontmatter says `CAD25004`, body says `CAD25002`.
2. Identify whether Syncrude/Mildred Lake should become a facility record.
3. Decide whether Canadian CAD jobs should live beside USA jobs in `03-jobs` or under a separate convention.
...
| 2026-07-05 | Closed as superseded — Jesse confirmed. Everything this note proposed for CAD25004 already happened via later reviews: job-number confirmed ([[2026-06-26-cad25004-routing-review]]), contradiction resolved ([[2026-06-26-cad25004-job-number-contradiction]]), canonical updates approved ([[2026-06-26-cad25004-candidate-canonical-updates]]), heater scaffold created (`02-facilities/Syncrude/Fort-McMurray-AB/7-2-F-1.md`). Nothing left to approve here. | Claude |

...
**Defect 2 — inbox and content-layer routing is assigned to a loop that does not run.** Line 62 says those layers "are owned by the Vault Capture Loop." Ownership of a queue by a stopped process is indistinguishable, from inside a run, from ownership by nobody. The vault's own entry point already states the replacement rule — route it when you write it, three-outcome logic unchanged, trigger gone — so the correct text exists; it just is not here. Line 64's claim that the capture loop appends scheduled run summaries to `change-log.md` is the same error in miniature.

**The 82-item inbox is not evidence against the shutdown, and must not be read that way.** The audit's one failed prediction was that the inbox would "begin draining on its own once the capture loop stops adding to it" (`2026-08-20-vault-architecture-audit-evidence.md:175`); it went 54 → 82 instead. But the growth *slowed*: counted from the `created:` dates of the notes sitting in `00-inbox/` today, 34 landed in the 12 days before the shutdown against 29 in the 18 days after — roughly 2.8/day down to 1.6/day. That is my count of surviving notes, not a stated figure, and it understates the pre-shutdown rate, since older notes had more opportunity to be routed out. The additions since are attended sessions writing capture notes, which is the intended workflow. What is missing is not the loop; it is that nothing was ever assigned to *drain* the pile, and the spec pointing at a stopped loop is part of why that gap stayed invisible.
...
**Recommendation: 1 and 2 approved as stated, and 3(b).** Proposals 1 and 2 correct statements that are false today with no judgment involved. On 3, re-ranking (a) writes the current configuration into the loop's permanent instruction, and that configuration is reversible by design — `system-workflow-reference.md:5` records that re-enabling a stopped loop is one `enabled: true` plus a row back in `LOOP_HEARTBEATS`. A sentence costs nothing and survives that reversal intact.

**Nothing in this note argues the 2026-08-21 shutdown was wrong, and the wording has been corrected where it implied otherwise.** The shutdown verdict is intact and independently supported: `2026-08-20-vault-architecture-audit-evidence.md:87` records the capture loop as a **net producer** — the inbox grew 21 → 54 *while it ran daily*, and its final run ingested 1 and harvested 2. This note's complaint is that the active spec was never told, not that the decision was bad.
...
**Confirmed twice, 2026-09-08 — this note's one open verification item is closed.** Jesse stated in session that the `vault-capture-loop` and `vault-prestaging-loop` tasks are disabled and that disabling them was his own decision. Independently, the scheduler itself was read the same day: `vault-capture-loop`, `vault-idea-research-loop` and `vault-prestaging-loop` all return `enabled: false` with `lastRunAt` 2026-08-21, while `vault-skill-drift-loop`, `vault-consolidation-loop`, `vault-review-loop` and `vault-audit-remeasure` remain enabled. The item was raised because the five vault sources agreeing on it all descend from the same 2026-08-21 shutdown and the ledger's silence is an absence rather than a read — agreement among derived notes is not corroboration. It now has both an owner's statement and a direct read of the system. The premise under Defects 1 and 2 is verified.

**A larger instance of the same defect was found while checking this**, and it is recorded in [[2026-09-08-inbox-routing-roi-handover]] rather than here: four *active* documents still instruct writers to drop notes in `00-inbox/` and name one of the stopped loops as the consumer that will route them — `idea-triage`'s Park disposition, `08-systems/mobile-field-access.md`, `knowledge-system-governance.md:151`, and six sites in `usadebusk-vault-ingest`. This note's Defect 2 is the consumer-side half of that; the producer side is larger and includes an operational trap on the field-capture path. Whoever applies DQ-032 should fix both sides in one pass.

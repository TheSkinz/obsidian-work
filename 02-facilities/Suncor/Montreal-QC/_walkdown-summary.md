---
type: reference
last-updated: 2026-07-07
tags: [walkdown, Suncor, Montreal]
---

# Suncor Montreal — Field Walk-Down Summary (5 Heaters)

Full detail and source drawings live in each heater's own card in this folder.

---

## B-101 — Vacuum Feed Heater
**Passes:** Radiant 2 (Double Mode). Convection 10 coils via Mk3/Mk4.
**Tube ID:** Radiant steps 10.02"/7.98"/6.07"/5.05"/4.03". Convection ~4.03" (unconfirmed). **Governing ID: 4.03".**
**Metallurgy:** Convection P5, Radiant P9 — mixed, both need passivation confirmation.
**Connections:** Conv 4"-300# RF ×2 (Mk3/Mk4). Radiant 10"-150# RF ×2 (Mk1/Mk2) — ⚠ verify rating, a 10"-300# label also appears on-source.
**Max Pig OD:** ~4.28"
**Flags:** Footprint 12 sqft, elev 24 ft, scaffolding by Suncor. Smart pig/X-ray ELECTED.

---

## B-102 — Crude Furnace
**Passes:** Convection 2 (Double Mode). Radiant 2 legs (Double Mode). **2 independent circuits — crossover removed for decoke, not one continuous path.**
**Tube ID:** Convection ~4.03". Radiant 6" run ~6.07", 8" run ~7.98"/8.07". **Two circuits, two different pig sizes.**
**Metallurgy:** Convection P5. Radiant: P5 (6" main) → P9 (short 8" stub) → 347H stainless (8" outlet).
**Connections:** Conv 4"-300# RF at Mk1/Mk3, plus Mk2/Mk4/Mk5. Radiant 8"-300# RF ×2.
**Max Pig OD:** Conv ~4.28". Radiant ~6.32".
**Flags:** N2 purge required before removing spool sections 1-5/11-15. Site supplies own soda ash/filter press setup — confirm scope split.

---

## B-1001 — Crude Oil Heater
**Passes:** 4 total (A/B/C/D), 2 circuits of 2 passes each (Double Mode per circuit) — #1=A+D, #2=B+C.
**Tube ID:** 4.086" (confirmed, uniform).
**Metallurgy:** SA213 9Cr-1Mo throughout — no mixing.
**Connections:** Circuit #1 4"-300# RF Mk3/Mk5. Circuit #2 4"-300# RF Mk4/Mk6. Manifold (Mk1/Mk2) ties to 10"-300# line, blinded for decoke.
**Max Pig OD:** ~4.34"
**Flags:** Full instrument removal required pre-decoke (doubled TCs) — more prep than the other four.

---

## B-103 — Preflash Reboiler
**Passes:** 1, single circuit — not Double Mode.
**Tube ID:** Not confirmed, est. ~4.0" — **confirm in field.**
**Metallurgy:** 5Cr-½Mo family (unconfirmed base tube grade).
**Connections:** Entrance 4"-300# RF ground level. Exit 4"-300# RF, 4th balcony, 35 ft up.
**Max Pig OD:** ~4.25" (estimated)
**Flags:** ⚠ 35 ft elevation entrance-to-exit — biggest access issue in the package. No smart pig/filtration election.

---

## B-151 — Convection Section (no radiant)
**Passes:** 1, single circuit — not Double Mode. No radiant section.
**Tube ID:** 4.03" (confirmed).
**Metallurgy:** A106 Gr B carbon steel — only CS heater in the package, no passivation.
**Connections:** Mk-1 IN, Mk-2 OUT, both 4"-300# WNRF. Mk-3 is a blinded bypass spool, not a pigging connection.
**Max Pig OD:** ~4.28"
**Flags:** Simplest heater in the package. One source sheet is mislabeled "B-102" in its title block — it's actually B-151.

---

## Cross-Heater Notes
- Temp decoke spools share spec UD1 (water/soda ash, 700 psig/370°F design, 1050 psig hydrotest) across all 5 heaters.
- Filtration TBD on B-102, B-1001, B-103, B-151. Only B-101 has an elected job option (smart pig/X-ray).
- Double Mode: B-102 (both circuits), B-1001 (both circuits), B-101 radiant. B-103 and B-151 are single-pass/standard.

...
- **walkinglabs/awesome-harness-engineering** — evaluated 2026-07-24, real and substantive (3.7k stars, CC0, ~130 curated links, spot-checked anchor entries resolve to genuine primary sources), but roughly 60% of it (Benchmarks, Runtimes) is built for people *shipping* agent products, not *operating* a single harness, and doesn't apply here. The ~10 links that do apply are worth keeping as reference (e.g. Anthropic's "Effective harnesses for long-running agents" on initializer agents, handoff artifacts, and self-verification across context windows). Not adopted as a standing reference, spot-useful only.
- (Placeholder — add other repos, videos, or articles evaluated as this area develops)

## Skill-value verification: the no-skill baseline test

A concrete design for testing whether a domain skill (e.g. `usadebusk-estimating`) actually helps, surfaced while evaluating the awesome-harness-engineering list's eval-methodology links: take several completed jobs with known ground truth (the actual submitted proposal), and re-run the intake/estimating task twice — once with the skill loaded, once with only the raw source package as input, no domain skill. Score both outputs against the real submitted proposal on deterministic, checkable-without-judgment items (did it flag required intake fields when data was missing, did it compute duration correctly, did it avoid inventing a rate instead of asking, did it apply the condition-matched-actuals rule instead of pricing a routine job off a crash row). The value isn't proving the skill helps in general — it obviously encodes real institutional knowledge no base model has — the value is catching where the skill's *phrasing* fails to prevent a known failure mode even with it loaded. Applies the same principle as [[prompt-engineering]]'s "test on real data before scaling" rule, specialized to skill authoring.

...
**3. `estimating-pricing.md:68` names the $58.00 line "Driver Travel".** (V3.) The estimating skill calls the same rate "Crew Travel (non-driver)" and states at `:66-69` that the $3.00/mile already includes driver labor, so drivers carry no separate travel-labor line. Applying the vault's label as written would double-bill driver travel. The evidence is clean and the fix is a one-word rename, but it is a row in the pricing table, so it is yours. Recommend renaming it to "Crew Travel (non-driver)" to match the skill.

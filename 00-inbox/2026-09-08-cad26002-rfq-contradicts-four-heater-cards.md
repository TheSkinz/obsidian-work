...
- **Which tender option were we awarded?** §.3 offers Option 1 (one furnace after another, 1 decoke truck + 1 filter press) and Option 2 (two furnaces in parallel, the 5th after, **2 decoke trucks + 2 filter presses**). Travis's 2026-09-08 plan runs two heaters per run — Option 2's sequence — on Trimax 6 / Support 6 / Press 2, which is Option 1's equipment count. There is **no quote note for this job anywhere in the vault**: no DSP number, no submitted price, no `type: quote` frontmatter for Suncor. The only Montreal quote in the estate is `DSP# 25011CAD Suncor Montreal April 2025.pdf`, which predates this package.

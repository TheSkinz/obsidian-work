...
**Tentative read:** A repo-relative path written in backticks is invisible to every check the vault has. `04-knowledge/knowledge-system-governance.md` records this as a known hazard under Terminal-Note Sweep (written 2026-07-29 in the capture-loop spec, moved here 2026-09-08 when that spec was retired — and **this citation was itself one of the two live instances that broke in the move**, which is the seed's own case proving itself); nothing has closed it; POINTER-DEAD covers only *absolute* recorded paths, and DEAD-LINK covers only `[[wikilinks]]`. Two live instances surfaced on 2026-08-24: `apps/pig-tracker/pig-tracker.html:393` is cited by approved unexecuted work in three notes with nothing protecting it, and `02-facilities/HF-Sinclair/Artesia-NM/H-2501.md:19` had been pointing at a file that moved to `archive/` — dead, and unnoticed.
